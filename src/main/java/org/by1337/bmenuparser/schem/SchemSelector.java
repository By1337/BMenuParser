package org.by1337.bmenuparser.schem;
...
import org.by1337.bmenuparser.render.RenderUtil;
        ...
        ClientCommandManager.DISPATCHER.register(LiteralArgumentBuilder.<FabricClientCommandSource>literal("//schem")
                .then(LiteralArgumentBuilder.<FabricClientCommandSource>literal("select")
                        .executes(ctx -> {
                            enabled = !enabled;
                            if (enabled) {
                                ctx.getSource().sendFeedback(new LiteralText("Используйте лкм и пкм чтобы выделить регион"));
                            } else {
                                ctx.getSource().sendFeedback(new LiteralText("Выделение территории отключено"));
                            }
                            return 1;
                        })
                )
                                ...
                                .executes(ctx -> {
                                    ...
                                    if (pos1 != null && pos2 != null) {
                                        try {
                                            ...
                                            t.printStackTrace();
                                            ctx.getSource().sendFeedback(new LiteralText("Не удалось сохранить схематику: " + t.getMessage()));
                                            return 1;
                                        }
                                        ctx.getSource().sendFeedback(new LiteralText("Схематика сохранена!"));
                                    } else {
                                ...
                                .executes(ctx -> {
                                    ...
                                    if (pos1 != null && pos2 != null) {
                                        ...
                                        if (player != null) {
                                            ...
                                            ctx.getSource().sendFeedback(new LiteralText("Регион увеличен на " + blocks + " блоков по направлению взгляда"));
                                            region.resize();
                                            pos1 = new BlockPos(region.minX, region.minY, region.minZ);
                                            pos2 = new BlockPos(region.maxX, region.maxY, region.maxZ);
                                        }
