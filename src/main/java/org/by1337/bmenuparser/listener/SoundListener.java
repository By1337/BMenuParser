...
import org.by1337.bmenuparser.event.SoundEventListener;
...
public class SoundListener {
    ...
    public void register() {
        ClientCommandManager.DISPATCHER.register(LiteralArgumentBuilder.<FabricClientCommandSource>literal("//sound_log")
                .executes(ctx -> {
                    enabled = !enabled;
                    if (enabled) {
                        ctx.getSource().sendFeedback(new LiteralText("Включено логирование звуков"));
                    } else {
                        ctx.getSource().sendFeedback(new LiteralText("Логирование выключено"));
                    }
                    return 1;
                })
        );
        ...
        NetworkEvent.SOUND_EVENT.register(new SoundEventListener() {
            ...
            public void on(SoundEvent event) {
                ...
                LiteralText text = new LiteralText("Звук: ");
                String sound = Sound.getBukkitNameOr(event.getSound().getId().getPath(), event.getSound().getId().getPath());
    ...
    public enum Sound {
        ...
        private static final Map<String, Sound> keyToBukkitName = new HashMap<>();
        ...
        public static Sound getSound(String key) {
            return keyToBukkitName.get(key);
        }

        public static String getBukkitNameOr(String key, String def) {
            Sound s = keyToBukkitName.get(key);
            if (s == null) {
                return def;
            }
            return s.name();
        ...
        static {
            for (Sound value : values()) {
                keyToBukkitName.put(value.getKey(), value);
            }
        }
    }
