    ...
    public void register() {
        ClientCommandManager.DISPATCHER.register(LiteralArgumentBuilder.<FabricClientCommandSource>literal("//particle_log")
                .executes(ctx -> {
                    enabled = !enabled;
                    if (enabled) {
                        ctx.getSource().sendFeedback(new LiteralText("Логирование партиклов включено!"));
                    } else {
                        ctx.getSource().sendFeedback(new LiteralText("Логирование партиклов выключено!"));
                    }
                    return 1;
                })
        );
        ...
        NetworkEvent.PARTICLE.register(packet -> {
            ...
            LiteralText text = new LiteralText("Спавн партикла: ");
